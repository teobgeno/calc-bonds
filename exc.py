...
def calculate_sum_product_0(spread, row, baseline, remaining_period):
    ...
    outSceen(str(remaining_period) + ' - ' + str(cashflow_fin) + ' - ' + str(100*discount_factor_fin)+ ' - ' + str(baseline_fin))
...
def calculate_sum_product_1(spread, row, baseline, remaining_period):
    total_period = 0
    rem_period = excel_round((days_between(row.iloc[7].strftime('%Y-%m-%d'),str(year-1) + '-12-31'))/ 365.25,9)
    ...
    for x in range(1, total_period):
        ...
        discount_factor_norm_plus = np.around((1/(1 + (baseline[x]) + spread)),9)
        discount_factor_norm = np.around((discount_factor_norm_plus[0] ** x),9)
        
        cashflow_list.append(cashflow_norm)
        discount_factor_list.append(discount_factor_norm)
        outSceen(str(x) + ' - ' + str(cashflow_norm) + ' - ' + str(100*discount_factor_norm) + '%' + ' - ' + str(baseline[x]) + ' - ' + str(discount_factor_norm_plus[0]))
    ...
    cashflow_list.append(cashflow_fin)
    discount_factor_list.append(discount_factor_fin)
    outSceen(str(remaining_period) + ' - ' + str(cashflow_fin) + ' - ' + str(100*discount_factor_fin)+ ' - ' + str(baseline_fin) + ' - ' + str(discount_factor_fin_plus[0]))
...
import_file = "import_bond_value.xlsx"
...
# 1 64
    if index == 64:
        ...
        rem_period = (days_between(row.iloc[7].strftime('%Y-%m-%d'),str(year-1) + '-12-31'))/ 365.25
        outSceen('-----------------------')
        #create_cachflow_table(rem_period, row)
        ...
        if row.iloc[10] == 1 or row.iloc[10] == 0:
            ...
            result = minimize(objective, initial_spread, args=(row, baseline, row.iloc[11], row.iloc[8], row.iloc[10]))
